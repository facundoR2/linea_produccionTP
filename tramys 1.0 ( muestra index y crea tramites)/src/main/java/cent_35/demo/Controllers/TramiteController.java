...
public class TramiteController {
    ...
    @GetMapping("/buscarTramite")
    public ResponseEntity<Object> getTramitePorNombre(@RequestParam String nombreTramite){
        try{
            TramiteDTO tramite = tramiteService.BuscarTramite(nombreTramite);

            if (tramite.getNombre() == null ) {
                // devolvera un codigo  404 implicando que no se encontró un tramite con ese nombre.
                return ResponseEntity.notFound().build();
            }
            //en caso positivo se devuelve un codigo 200 con el tramite encontrado.
            return ResponseEntity.ok(tramite);
        }catch (Exception a) {
            a.printStackTrace();
            return ResponseEntity.status(500).build(); //error 500 (error del lado del server)
        }
