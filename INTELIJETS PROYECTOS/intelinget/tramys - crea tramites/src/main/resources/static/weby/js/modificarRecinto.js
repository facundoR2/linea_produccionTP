const app = Vue.createApp({
    ...
    mounted(){
        this.initializeMap(); //inicializa el mapa al cargar el componente.
        
        const direccionInput = document.getElementById('direccion-input'); 
        if(direccionInput) {
            direccionInput.addEventListener('keydown',(Event) => {
                if(Event.key === 'Enter') { //captura el enter en el input de direccion.
    ...
    methods: {
        deleteRecinto(direccion) {
            if(confirm(`¿Estás seguro de que quieres eliminar el recinto en ${direccion}?`)) {
                axios.delete(`/api/recinto/eliminarRecinto/${direccion}`)
                .then(response => {
                    console.log('Recinto eliminado exitosamente', response.data);
                })
                .catch(error => {
                    console.error('Error al eliminar el recinto:', error);
                    alert('Hubo un error al eliminar el recinto. Por favor, intenta de nuevo');
                });
            }
        },
        initializeMap(){
